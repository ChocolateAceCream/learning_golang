...
func redisDataCol(conn redis.Conn) {
	...
	for deviceNum, deviceInfo := range deviceAll {
		wg.Add(1)
		go deviceDataProc(deviceInfo, deviceNum, *node)
	}
	wg.Wait()
...
func deviceDataProc(deviceInfo string, deviceCode string, nodeExc string) {
	...
	if strings.Contains(deviceInfo, "nodeCode") {
		...
		if devStatus == "9" {
			...
			if updateTimeStr != "" {
				...
				if time.Now().UnixNano()-updateTime > 30000 {
					...
					if err != nil {
						log.Println("delete device info fail")
					}
					_, err = conn.Do("HDEL", "DEVICE#HEART#BEAT", deviceCode)
					if err != nil {
						log.Println("delete device info fail")
					}
		...
		if nodeCode == nodeExc {
			...
			//流质量开关判断
			_, err = redis.String(conn.Do("HGET", "STREAM-QUALITY-DEVICES-HASH-KEY", deviceCode+"#"+deviceCode))
			if err == nil {
				qualityNum++
			}
...
func rc() {
	...
	redisDataCol(conn)
	log.Printf("quality open num : %d", qualityNum)
